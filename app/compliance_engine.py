...
# Words/phrases that mean the sentence is DENYING a right rather than
# granting it. Kept deliberately simple (substring match) rather than full
# NLP negation detection -- good enough for well-formed policy clauses, easy
...
# which one it found. (Unchanged from the original -- no negation logic
# here anymore.)
...
# Only called AFTER a sentence has already passed both the semantic
# threshold and the keyword check. Looks at the whole sentence for a
# negation cue, to catch cases like "Your data cannot be deleted, even
# upon request" -- which would otherwise pass both checks above.
...
def analyze_policy(policy_text_sentences: List[str]) -> List[Dict]:
    ...
    for section in PDPA_SECTIONS:
        ...
        # NEW: only now, on this one winning sentence, check for negation.
        # A sentence can pass both the semantic AND keyword checks while
        # still be DENYING the right (e.g. "Your data cannot be deleted,
        # even upon request") -- this catches that case.
        is_violation = False
        if matched_keyword and _contains_negation(policy_text_sentences[chosen_idx]):
            is_violation = True
            matched_keyword = None  # doesn't count toward keyword_score anymore
